...
_headerColumns = "Round Minimals;Subject; Subject Round awnsers;Subject round payouts";
...
//print(JSON.stringify(_completedTaskScore.scoringData));

var roundData = [];


for(var i=0; i < rounds.length; i++) {
    if(rounds[i].roundMinimal == 9999999) {
        roundData.push(0);
    }else {
        roundData.push(rounds[i].roundMinimal);
    }
}
...
for (var subject in userArray) {
    var su = userArray[subject]
    _exportRecordLines.push(JSON.stringify(roundData)
                            + ";" + su.user_external_id
                            + ";" + JSON.stringify(su.roundOptions)
                            +  ";"+ JSON.stringify(su.roundPayouts))
}

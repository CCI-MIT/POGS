class Wackamole {
    ...
    setupGrid(title, teammates, whackBluePrint) {
        ...
        $(document).on('keydown', self.handleOnKeyDown.bind(self));
        ...
        switch (e.key) {
            case 'ArrowUp':
                ...
                if(this.myIndex - 10 < 0){
                    this.myIndex = this.myIndex + 90;//wrap around
                } else {
                    this.myIndex= this.myIndex -10;
                }
                this.handleMouseMove(this.myIndex);
                break;
            case 'ArrowDown':
                // down arrow
                if(this.myIndex + 10 > 99){
                    this.myIndex = this.myIndex - 90;//wrap around
                }else {
                    this.myIndex= this.myIndex + 10;
                }
            ...
            case 'ArrowLeft':
                ...
                //[80,89]
                //[90,99]
                if(this.myIndex% 10 - 1 < 0 ){
                    this.myIndex = (this.myIndex/10)*10 + 9;//wrap around
                }else {
            ...
            // right arrow
                ...
                //[80,89]
                //[90,99]
                if(this.myIndex% 10 + 1 >= 10 ){
                    this.myIndex = this.myIndex - 9;//wrap around
                }else {
                    this.myIndex= this.myIndex +1;
                }
                this.handleMouseMove(this.myIndex);
            ...
            case ' ':
                //Space bar
                console.log("ESPACO")
                this.handleCellOnClick(this.myIndex);
    ...
    handleMouseMove(myPosition) {
        //update current CSS in file
        $(".whack_gametable-cell").removeClass('positioned');
        $("#whack_cell" + myPosition).addClass('positioned');
        // broadcast mouse move
        this.pogsPlugin.saveCompletedTaskAttribute('playMove',myPosition, 0.0, 0, false);
    }
    handleCellOnClick(cell) {
        var self = this;
